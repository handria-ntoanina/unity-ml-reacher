...
class Gaussian(nn.Module):
    def __init__(self, state_size, action_size, seed):
        ...
        self.critic = FullyConnected([state_size, 600, 400, 1], seed)
        self.critic_target = FullyConnected([state_size, 600, 400, 1], seed)
        soft_update(self.critic, self.critic_target, 1)
        
        # Need to check if this is adjusted during backpropagation or not
        self.std = torch.ones(1, action_size, requires_grad=True)
        self.std2 = torch.ones(1, action_size)
...
#         print([values[i].__class__.__name__ for i in range(len(values))])
#         print(scalar_idxs)
